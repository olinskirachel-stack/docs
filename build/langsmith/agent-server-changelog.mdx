...
- Switched the Python queue worker to use core `Runs.next()` to fix serialization issues in the gRPC layer.
- Fixed a bug in the long query monitor by utilizing Python PG.
- Improved the stream handling in `a2a` implementation to prevent flakiness in quick tests.
</Update>
